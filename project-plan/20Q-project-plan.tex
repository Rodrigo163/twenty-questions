...
For \textit{Hair}, this value is $abs(1 - 1.33) = 0.33$, which is greater than $0.18$, so given the choice between these two features, we would choose to split the input space on \textit{Predator} rather than \textit{Hair}.
