...
We decided on working with the SwDA to be able to investigate structures of polar questions and to use those as a reference point for the upcoming question generation part. Since the 20Q gameplay is based exclusively on the use of Yes-No questions, we concentrated on extracting Part of Speech (POS) tags of question marked by the act-tag "qy" in the SwDA. To perform pre-processing on the corpus we firstly created a pandas dataframe containing the information that is needed for our purpose (question index, act-tag, text, and POS tags). Subsequently we generated an additional column containing POS tags based on the spaCy library which are easier to handle, due to their simplified structure. Table \ref{tab:swda_preprocessing} shows the first 5 rows of the final dataframe.
...
{\small \texttt{
	\begin{tabularx}{\linewidth}{ccLLL} 
		...
		83 & qy & Were you -- & Were/VBD you/PRP --/: & ['VERB', 'PRON']  \\
		46 & qy & Are you in Texas?/ & Are/VBP you/PRP in/IN Texas/NNP ?/. & ['VERB', 'PRON', 'ADP', 'PROPN'] \\
		29 & qy & I probably would |have done, \newline {D you know, } just… & I/PRP probably/RB would/MD \newline have/VB done/VBN ,/... & ['PRON', 'ADV', 'VERB', \newline 'VERB', 'VERB', 'NOUN'...] \\
		1 & qy & Are you a Vietnam veteran, Dudley? \newline <Music>. / & Are/VBP you/PRP a/DT \newline Vietnam/NNP veteran/NN ,/... & ['VERB', 'PRON', 'DET', \newline 'PROPN', 'NOUN', 'PROP...] \\
		5 & qy & Do you have family who \newline were in the Vietnam War? / & Do/VBP you/PRP have/VB \newline family/NN who/WP were/V... & ['VERB', 'PRON', 'VERB', \newline 'NOUN', 'PRON', 'VERB...] \\
		\bottomrule
	\end{tabularx}
}}
\end{center}
...
Non-informative POS tags such as "PUNCT" (i.e. punctuation) and "SPACE" (i.e. blank spaces) were removed from the dataframe. In order to investigate the general structure of "qy" questions, we concentrated on the beginning of those sentences and created a dictionary consisting of the first five POS tags of each sentence and the frequency of each unique POS tag pattern. This gave us 1332 unique POS tag combinations, where the most frequent ones follow the structure $VERB \rightarrow{PRON} \rightarrow{VERB} \rightarrow{...} $ (see Table \ref{tab:pos_patterns}). Accordingly, for the 20Q gameplay we will generate questions such as: "Does it have (a) tail?", "Does it live in the ocean?", "Is it big?". 
...
{\small \texttt{
	\begin{tabular}{lr}
		\toprule
		Occurrences & POS\_Tags \\ \midrule
	    153 & (VERB, PRON, VERB, DET, NOUN)\\
		90 & (VERB, PRON)\\
		74 & (VERB, PRON, CCONJ, VERB, PRON)\\
...
\end{center}
\caption{Unique POS tag patterns and their frequency counts, extracted from SwDA Corpus.}
\label{tab:pos_patterns}
\end{table}

The purpose of our ask\_about\_feature function inside the 20Q gameplay is to print out a natural language question based on a given feature name. Thanks to the feedback we received in class about our project we decided on adding biased questions too. In order to chose a biased or non-biased question the arguments passed to this functions are (i) the feature name itself, (ii) a majority value which represents the majority value for a given feature (either 0 or 1), and (iii) an extremeness value which symbolizes how out-of-balanced the values for a given feature are. Depending on whether the value of extremeness reaches a certain threshold (in our case 0.65), a biased question is chosen over the non-biased version, since our model’s certainty about this value is high enough to ask a biased question. We firstly considered implementing not only positive-anchored biased questions (i.e. "Your animal is big, isn’t it?"), but also negative-anchored biased questions (i.e. "Your animal doesn’t have feathers, does it?"), but we realized that those elicit ambiguous responses which could negatively affect the functioning of our model. 

\begin{enumerate}
  \item Pre-processing: For the question generation itself, we had to perform pre-processing of the feature names by replacing the underscores and partitioning the feature names consisting of x-words into an x-tuple (i.e. in\_the\_ocean gives ("in," ","the"," ", "ocean"), have\_hair gives ("have"," ","hair").
  ...
  \item Question Generation: Depending on the POS tag attained in the previous step, choose one of the pre-defined question patterns (see Table  \ref{tab:question_patterns}). 

\end{enumerate}

The question patterns were created based on the results obtained in the pre-processing file of the SwDA Corpus. Constructing proper questions was especially challenging with respect to the feature names containing a verb, given that it had to be in the third person singular form in order make the question syntactically coherent. To access the third person singular form we made use of the Pattern library (citation), a useful natural language processing library for Python which contains a function, namely lexeme, that provides all lexeme forms for a given word, in our case a verb. This is exactly what we needed in order to generate proper questions for feature names containing a verb. 
...
{\small \texttt{
	\begin{tabularx}{\linewidth}{cLLLLL} 
		\toprule
		 & VERB (+ed), ADJ, ADV & NOUN (pl), DET & NOUN (sg) & VERB, AUX & ADP \\ \midrule
